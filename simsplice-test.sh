...
find * -maxdepth 0 -type d |parallel -q @minimap2 bash -xc 'cd "$0" && minimap2 --eqx -aYx sr -t "$(nproc)" assembly.fasta <(gzip -cd *_illumina.*_1.fastq.gz) <(gzip -cd *_illumina.*_2.fastq.gz) |samtools view -Sbu - |samtools sort -@ "$(nproc)" - -o illumina_original.bam && samtools index illumina_original.bam'

find * -maxdepth 0 -type d |parallel -q @minimap2 bash -xc 'cd "$0" && minimap2 --eqx -aYx map-ont -t "$(nproc)" assembly.fasta <(gzip -cd ONT_original.fastq.gz) |samtools view -Sbu - |samtools sort -@ "$(nproc)" - -o ONT_original.bam && samtools index ONT_original.bam'
